...
# LOG_FILE = '/mnt/vol/log/cron/amz_upload_stock_and_price_update.py___%s.log' % datetime.today().strftime('%Y_%m')
logging.basicConfig(
    level=logging.INFO,
...
def get_xml_content(store, now):
    ...
    rows = Connection.odoo_execute("""
        SELECT PL.name, PTL.wh_qty, PTL.vendor_qty FROM product_template_listed PTL
...
def get_min_and_max_prices_to_upload(store):
    rows = Connection.odoo_execute("""
        SELECT PL.name, PTL.wh_qty, PL.current_min_price, PL.current_max_price, REP.name as repricer, PTL.total_min_cost FROM product_template_listed PTL
        LEFT JOIN product_listing PL ON PL.product_tmpl_id = PTL.product_tmpl_id
    ...
    if rows:
        for row in rows:
            ...
            if not row['current_min_price'] or "{0:.2f}".format(float(row['current_min_price'])) != "{0:.2f}".format(
                ...
                rows_to_upload.append((row['name'], "{0:.2f}".format(min_price), "{0:.2f}".format(max_price), repricer))
...
def amazon_upload_pricing_to_xsellco(store, now):
    ...
    while batch <= batches:
        ...
        ftp.storbinary("STOR repricer_%s.zip" % datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), open(to_upload, "rb"),
                       1024)
        ftp.quit()
        batch += 1
