...
class BulkDataExchangeService( LMSCall ):
    ...
    def __init__(self, environment, credentials ):
        LMSCall.__init__( self, environment, credentials)
...
class CreateUploadJob( BulkDataExchangeService ):
    ...
    def __init__(self, environment, credentials ):
        BulkDataExchangeService.__init__( self, environment, credentials )
...
class UploadFile( FileTransferService ):
    ...
    def __init__(self, environment, credentials ):
        FileTransferService.__init__( self, environment, credentials )
...
class GetJobs( BulkDataExchangeService ):
    ...
    def buildRequest( self, jobType, jobStatus ):
        '''
        ...
        if jobType:
            request += '<jobType>%s</jobType>' %jobType
        if jobStatus:
            request += '<jobStatus>%s</jobStatus>' %jobStatus
...
class DownloadFile( FileTransferService ):
    ...
    def _parse_response( self ):
        ...
        fp = open('/tmp/data_responses.zip', 'wb')
        fp.write( data )
        fp.close()
...
class startDownloadJob( BulkDataExchangeService ):
    ...
    def __init__(self, environment, credentials ):
        ...
        self.args = []
        self.allowable_jobtypes = ('ActiveInventoryReport', 'FeeSettlementReport', 'OrderReport', 'SoldReport')


    def buildRequest( self, jobType, uuid, listingType=None, createdFromDaysAgo=30, now=None ):
        '''
        This function builds the request string for a abortJob api call with
        the given jobType
        ...
        if jobType in self.allowable_jobtypes:
            ...
            if listingType in ['Auction', 'AuctionAndFixedPrice', 'FixedPrice']:
                ...
                request += '</activeInventoryReportFilter>\r\n'
                request += '</downloadRequestFilter>\r\n'
            if jobType == 'OrderReport':
                if not now:
                    now = datetime.utcnow()
                dt_format = '%Y-%m-%dT%H:%M:%S'
                create_time_to = now.strftime(dt_format)
                request += '<downloadRequestFilter>\r\n'
                request += '<orderReportFilter>\r\n'
                if createdFromDaysAgo:
                    create_time_from = (now - timedelta(days=createdFromDaysAgo)).strftime(dt_format)
                    request += '<createTimeFrom>%s</createTimeFrom>\r\n' % create_time_from
                request += '<createTimeTo>%s</createTimeTo>\r\n' % create_time_to
                # request += '<orderStatus>Completed</orderStatus>\r\n'
                request += '<version>1045</version>\r\n'
                request += '</orderReportFilter>\r\n'
                request += '</downloadRequestFilter>\r\n'
            request += '<UUID>%s</UUID>\r\n' % uuid
