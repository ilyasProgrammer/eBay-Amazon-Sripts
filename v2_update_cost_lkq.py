...
LOG_FILE = os.path.join(logs_path + 'cost_lkq_%s.log') % datetime.today().strftime('%Y_%m')
...
def go_LKQ():
    qry = """SELECT DISTINCT Inv.PartNo AS LAD, Inv.InventoryID, Inv_.MfgID, Inv_.PartNo AS ALT, w.CustomerPrice
                ...
                LEFT JOIN C2C.dbo.Warehouse w
                ON w.partNumber = Inv_.PartNo
                WHERE Inv.InventoryID NOT IN (SELECT InventoryID FROM AutoPlus.dbo.InventoryPiesKit)
                AND Inv_.MfgID in (17,21)
                AND w.CustomerPrice IS NOT NULL
                AND Inv.MfgID = 1
    ...
    for l in ap_lines:
        qry = """SELECT ps.id, ps.name as vendor, ps.product_code, pt.name as LAD 
                 ...
                 WHERE ps.name = 8 AND ps.product_code = '%s' AND pt.name = '%s'""" % (l['ALT'], l['LAD'])
        got_line = Connection.odoo_execute(qry)
        if got_line:
            if len(got_line) == 1:
                qry = """UPDATE product_supplierinfo SET price = %s WHERE id = %s """ % (float(l['CustomerPrice']), got_line[0]['id'])
            ...
            if not res:
                ...
                new_prod = rpc.create('product.template', {'name': l['LAD'],
                                                           ...
                                                           'description_sale': 'created by v2_update_cost_lkq.py'})
                product_id = new_prod
                if product_id:
            ...
            qry = """INSERT INTO product_supplierinfo 
                     ...
                     VALUES (8, '%s', %s, %s, 0, 3, 0, '%s', '%s', 1, 1) """ % (l['ALT'], product_id, float(l['CustomerPrice']), create_date, create_date)
            res = Connection.odoo_execute(qry, commit=True)
            logging.info('Created %s %s', l['ALT'], float(l['CustomerPrice']))
